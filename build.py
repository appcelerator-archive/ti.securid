...
def compile_js(manifest,config):
	js_file = os.path.join(cwd,'assets','ti.securid.js')
	...
	f = os.path.join(cwd,'Classes','TiSecuridModuleAssets.m')
	c = open(f).read()
	idx = c.find('return ')
	before = c[0:idx]
	after = """
...
ignoreFiles = ['.DS_Store','.gitignore','libTitanium.a','titanium.jar','README','ti.securid.js']
...
def package_module(manifest,mf,config):
	...
	for dn in ('assets','example','platform','lib'):
